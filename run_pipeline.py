...
def main() -> None:
    ...
    try:
        ...
        # Розкоментуй, коли готовий чекати довше (десятки хвилин через rate limit):
        # import collect_releases_by_genre
        # releases_df = collect_releases_by_genre.collect_all()
        # releases_df.to_sql("releases_by_genre_year_raw", conn, if_exists="replace", index=False)

        recompute_releases_by_genre_year(conn)
        conn.commit()
